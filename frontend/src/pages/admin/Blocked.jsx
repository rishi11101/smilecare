...
export default function AdminBlocked() {
  ...
  const fetchBlocked = async () => {
    try {
      const res = await api.get('/api/slots/blocked');
      setBlocked(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await api.post('/api/slots/block', form);
      setForm({ date: '', reason: '' });
      fetchBlocked();
    } catch (err) {
  ...
  const handleUnblock = async (date) => {
    ...
    try {
      await api.delete(`/api/slots/block/${date}`);
      fetchBlocked();
    } catch {
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            ...
            <input
              ...
              className="border-2 border-gray-100 rounded-xl px-4 py-2.5 text-slate font-body text-sm focus:outline-none focus:border-navy transition bg-white"
            />
          </div>
          ...
          <button
            onClick={handleAdd}
            disabled={saving}
            className="inline-flex items-center gap-2 bg-navy text-white font-display font-700 text-sm px-5 py-2.5 rounded-xl hover:bg-navy-dark transition-colors disabled:opacity-60 whitespace-nowrap"
          >
            ...
            {upcoming.map(b => (
              <div key={b.id} className="flex items-center gap-4 px-5 py-4">
                ...
                <button
                  ...
                  className="p-2 rounded-lg text-slate-light hover:bg-red-50 hover:text-red-500 transition-colors flex-shrink-0"
                  title="Unblock"
                >
                  <Trash2 size={15} />
                </button>
