...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([api.get('/api/appointments/stats'), api.get('/api/appointments?limit=5')])
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-6">
        ...
        <p className="font-body text-slate-light text-sm mt-0.5">{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
      </div>
            ...
            {recent.map(a => (
              <div key={a.id} className="px-5 py-3.5 flex items-center gap-3 sm:gap-4">
                <div className="w-8 h-8 rounded-full bg-navy flex items-center justify-center flex-shrink-0">
                  <span className="font-display font-700 text-white text-xs">{a.patient_name.charAt(0).toUpperCase()}</span>
                </div>
                ...
                <div className="text-right flex-shrink-0 hidden sm:block">
                  <p className="font-body text-slate-mid text-xs">{formatDate(a.appointment_date)}</p>
